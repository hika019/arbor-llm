...
## 概要
BLT (Byte Latent Transformer) アーキテクチャに BitNet b1.58 (ternary量子化) を統合した、トークナイザ不要・ternary量子化のLLMを RTX 4090 一枚でゼロから学習するプロジェクト。
...
| 項目 | 値 |
|---|---|
| モデル種別 | BitNet b1.58系（W1.58A8: weight 1.58bit, activation 8bit） |
...
| 依存管理 | `requirements.txt` を版固定で管理（PyTorch は CUDA 版を index-url 指定） |
...
| Compile | `torch.compile(mode="max-autotune")`、BitLinear は静的 shape を維持 |
...
| Checkpoint | **非同期保存**（学習スレッドをブロックしない）、safetensors |
...
| 保持ポリシー | 直近 K 個 のローテーション + **best loss** + **最終** の3系統を常時保持 |
...
| アトミック保存 | tmpファイルへ書き出してから rename（途中で kill されても破損しない） |
...
4. すべて `requirements.txt` に版固定で記録、`.venv/` は `.gitignore` に追加
...
1. **BLT公式（facebookresearch/blt）を fork** し、自リポジトリにサブモジュールまたはコピーで取り込む
   - 推奨: fork した GitHub リポジトリを `third_party/blt/` に submodule として配置
...
3. **速度最適化（RTX 4090で1B学習を現実的時間に収めるため必須）**:
   ...
   - **torch.compile**（mode="max-autotune"、BitLinear が dynamic shape にならないよう静的化）
...
2. **保存形式**
   ...
   - 1 checkpoint = 1 ディレクトリ:
     ...
     ```
   - **アトミック保存**: `<dir>.tmp/` に書いて `os.rename` で確定、途中killでも壊れない
   - 保持ポリシー: 直近 K=3 + best + 1万step毎の長期保存
3. **再開**
...
├── README.md
├── third_party/
│   └── blt/                        # facebookresearch/blt の fork (submodule)
├── src/
│   ├── model/
